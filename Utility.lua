...
contentContainer.CanvasSize = UDim2.new(0, 0, 0, 800) -- Adjusted for new button
...
-- State and coroutine management
local scriptStates = {
    ...
    AutoLeave = { enabled = false, coroutine = nil, connection = nil } -- เพิ่ม AutoLeave
}

-- Priority script to ensure specific scripts take precedence
...
local configSystem = {
    ...
    saveConfig = function(self)
        ...
        if not success then
            warn("Failed to save config for " .. player.Name .. ": " .. tostring(err))
        end
    end,
    ...
    loadConfig = function(self)
        ...
        if not success then
            warn("Failed to load config for " .. player.Name)
            return false
        end
...
local function createToggleButton(name, layoutOrder, description)
    ...
    button.MouseLeave:Connect(function()
        if scriptStates[name].enabled then
...
local buttons = {
    AutoUpgrade = { button = nil, status = nil, knob = nil, bg = nil },
    RetryMap = { button = nil, status = nil, knob = nil, bg = nil },
    AutoPlay = { button = nil, status = nil, knob = nil, bg = nil },
    VotePass = { button = nil, status = nil, knob = nil, bg = nil },
    InspectFinished = { button = nil, status = nil, knob = nil, bg = nil },
    StandardSummon = { button = nil, status = nil, knob = nil, bg = nil },
    ChallengeAndEasterEvent = { button = nil, status = nil, knob = nil, bg = nil },
    VoteNext = { button = nil, status = nil, knob = nil, bg = nil },
    RangerAndEasterEvent = { button = nil, status = nil, knob = nil, bg = nil },
    GogetaMadaraAndZ10 = { button = nil, status = nil, knob = nil, bg = nil },
    GogetaAndZ10 = { button = nil, status = nil, knob = nil, bg = nil },
    MadaraAndZ10 = { button = nil, status = nil, knob = nil, bg = nil },
    AllRangerAndAllChallenge = { button = nil, status = nil, knob = nil, bg = nil },
    GhoulRangerAndGhoulStory = { button = nil, status = nil, knob = nil, bg = nil },
    AutoLeave = { button = nil, status = nil, knob = nil, bg = nil } -- เพิ่ม AutoLeave
}

buttons.AutoUpgrade.button, buttons.AutoUpgrade.status, buttons.AutoUpgrade.knob, buttons.AutoUpgrade.bg = 
    createToggleButton("AutoUpgrade", 1, "Automatically upgrades units")
buttons.RetryMap.button, buttons.RetryMap.status, buttons.RetryMap.knob, buttons.RetryMap.bg = 
    createToggleButton("RetryMap", 2, "Automatically votes to retry maps")
buttons.AutoPlay.button, buttons.AutoPlay.status, buttons.AutoPlay.knob, buttons.AutoPlay.bg = 
    createToggleButton("AutoPlay", 3, "Toggles auto-play functionality")
buttons.VotePass.button, buttons.VotePass.status, buttons.VotePass.knob, buttons.VotePass.bg = 
    createToggleButton("VotePass", 4, "Automatically votes to pass")
buttons.InspectFinished.button, buttons.InspectFinished.status, buttons.InspectFinished.knob, buttons.InspectFinished.bg = 
    createToggleButton("InspectFinished", 5, "Handles inspection completion")
buttons.StandardSummon.button, buttons.StandardSummon.status, buttons.StandardSummon.knob, buttons.StandardSummon.bg = 
    createToggleButton("StandardSummon", 6, "Auto summons with gem management")
buttons.ChallengeAndEasterEvent.button, buttons.ChallengeAndEasterEvent.status, buttons.ChallengeAndEasterEvent.knob, buttons.ChallengeAndEasterEvent.bg = 
    createToggleButton("Challenge & Easter", 7, "Farms Challenge and Easter events")
buttons.VoteNext.button, buttons.VoteNext.status, buttons.VoteNext.knob, buttons.VoteNext.bg = 
    createToggleButton("VoteNext", 8, "Automatically votes for next map")
buttons.RangerAndEasterEvent.button, buttons.RangerAndEasterEvent.status, buttons.RangerAndEasterEvent.knob, buttons.RangerAndEasterEvent.bg = 
    createToggleButton("Ranger & Easter", 9, "Farms Ranger stages and Easter events")
buttons.GogetaMadaraAndZ10.button, buttons.GogetaMadaraAndZ10.status, buttons.GogetaMadaraAndZ10.knob, buttons.GogetaMadaraAndZ10.bg = 
    createToggleButton("Gogeta-Madara & Z10", 10, "Farms Namek/Naruto Ranger stages and Z10")
buttons.GogetaAndZ10.button, buttons.GogetaAndZ10.status, buttons.GogetaAndZ10.knob, buttons.GogetaAndZ10.bg = 
    createToggleButton("Gogeta & Z10", 11, "Farms Namek Ranger stages and Z10")
buttons.MadaraAndZ10.button, buttons.MadaraAndZ10.status, buttons.MadaraAndZ10.knob, buttons.MadaraAndZ10.bg = 
    createToggleButton("Madara & Z10", 12, "Farms Naruto Ranger stages and Z10")
buttons.AllRangerAndAllChallenge.button, buttons.AllRangerAndAllChallenge.status, buttons.AllRangerAndAllChallenge.knob, buttons.AllRangerAndAllChallenge.bg = 
    createToggleButton("AllRangerAndAllChallenge", 13, "Farms all Ranger Stages then Challenge Mode")
buttons.GhoulRangerAndGhoulStory.button, buttons.GhoulRangerAndGhoulStory.status, buttons.GhoulRangerAndGhoulStory.knob, buttons.GhoulRangerAndGhoulStory.bg = 
    createToggleButton("GhoulRanger & GhoulStory", 14, "Farms Tokyo Ghoul Ranger stages and Chapter 10")
buttons.AutoLeave.button, buttons.AutoLeave.status, buttons.AutoLeave.knob, buttons.AutoLeave.bg = 
    createToggleButton("AutoLeave", 15, "Automatically leaves after game ends") -- LayoutOrder 15 เพื่อให้อยู่ก่อน Save Config
-- Save Config button
    ...
    if scriptName == "GogetaMadaraAndZ10" or scriptName == "AllRangerAndAllChallenge" then
        ...
        for name, state in pairs(scriptStates) do
            if name ~= "GogetaMadaraAndZ10" and name ~= "AllRangerAndAllChallenge" and state.enabled then
                local scriptFunc = ({
                    AutoUpgrade = autoUpgradeScript,
                    RetryMap = retryMapScript,
                    AutoPlay = autoPlayScript,
                    VotePass = votePassScript,
                    InspectFinished = inspectFinishedScript,
                    StandardSummon = standardSummonScript,
                    ChallengeAndEasterEvent = challengeAndEasterEventScript,
                    VoteNext = voteNextScript,
                    RangerAndEasterEvent = rangerAndEasterEventScript,
                    GogetaAndZ10 = gogetaAndZ10Script,
                    MadaraAndZ10 = MadaraAndZ10Script,
                    AllRangerAndAllChallenge = allRangerAndAllChallengeScript,
                    GhoulRangerAndGhoulStory = GhoulRangerAndGhoulStoryScript
                })[name]
                if scriptFunc then
...
local function createAndStartChallengeRoom()
    -- ตรวจสอบว่าผู้เล่นอยู่ในโหมด Challenge หรือไม่
    ...
    if replicatedStorage.PlayRoom:FindFirstChild(player.Name, true) then
        ...
        sendEvent("Remove")
        task.wait(1) -- รอให้ออกจากห้องก่อน
    end

    -- 1. ตั้งค่า World, Chapter, และ Difficulty (hard-coded ชั่วคราว)
    local world = "World1" -- ปรับตามชื่อโลกที่ถูกต้อง
    local chapter = "Challenge1" -- ปรับตามชื่อด่านที่ถูกต้อง
    local difficulty = "Hard" -- ปรับตามความยากที่มีในเกม
    ...
    task.wait(1) -- รอให้เซิร์ฟเวอร์ประมวลผล
    ...
    -- 3. ตรวจสอบว่าห้องถูกสร้างหรือไม่
    local roomCreated = false
    local startTime = tick()
    while tick() - startTime < 40 do -- รอสูงสุด 40 วินาที
    ...
    -- 4. ถ้าห้องถูกสร้างแล้ว ให้เริ่มเกม
    if roomCreated then
...
local function startEasterEvent()
    if isPlayerInSomeMode("Event") then return true end
    ...
    if not sendEvent("Easter-Event") then return false end
    task.wait(0.5)
    ...
    if not sendEvent("Start") then return false end
...
local function fireServerEvent(event, args)
    if event then
        ...
        DebugPrint("Fired Remote Event: " .. args[1])
    else
...
local function enableGui(guiPath, guiName)
    ...
    if gui then
        gui.Enabled = true
        DebugPrint("Enabled GUI: " .. guiName)
    else
...
if not playRoomEvent or not playRoomUIEvent then
    warn("Unable to access Remote Events")
end
...
local function autoJoinStage(world, chapter)
    ...
    if gameStage then
        ...
        warn("GameStage not found in PlayRoom")
    end
    ...
    if gameSubmit then
        ...
        warn("Game_Submit not found in PlayRoom")
    end
...
local function gogetaMadaraAndZ10Script()
    ...
    while scriptStates.GogetaMadaraAndZ10.enabled do
        DebugPrint("GogetaMadaraAndZ10 loop iteration, enabled: " .. tostring(scriptStates.GogetaMadaraAndZ10.enabled))
        local allCleared = true
        for i, stage in ipairs(gogetaMadaraStages) do
            ...
            if not isRangerStageCleared(stage.chapter) then
                ...
                if success then
                    ...
                    DebugPrint("Exited stage loop for " .. stage.chapter .. ", in Ranger Stage: " .. tostring(isPlayerInSomeMode("Ranger Stage")))
                    task.wait(5)
                else
    ...
    DebugPrint("GogetaMadaraAndZ10 script stopped, enabled: " .. tostring(scriptStates.GogetaMadaraAndZ10.enabled))
...
local function gogetaAndZ10Script()
    ...
    while scriptStates.GogetaAndZ10.enabled do
        DebugPrint("GogetaAndZ10 loop iteration, enabled: " .. tostring(scriptStates.GogetaAndZ10.enabled))
        local allCleared = true
    ...
    DebugPrint("GogetaAndZ10 script stopped, enabled: " .. tostring(scriptStates.GogetaAndZ10.enabled))
...
local function MadaraAndZ10Script()
    ...
    while scriptStates.MadaraAndZ10.enabled do
        DebugPrint("MadaraAndZ10 loop iteration, enabled: " .. tostring(scriptStates.MadaraAndZ10.enabled))
        local allCleared = true
    ...
    DebugPrint("MadaraAndZ10 script stopped, enabled: " .. tostring(scriptStates.MadaraAndZ10.enabled))
...
local function challengeAndEasterEventScript()
    ...
    while scriptStates.ChallengeAndEasterEvent.enabled do
        ...
        if hasRangerCrystal then
            ...
            if not scriptStates.ChallengeAndEasterEvent.easterEventCoroutine then
...
local function GhoulRangerAndGhoulStoryScript()
    ...
    while scriptStates.GhoulRangerAndGhoulStory.enabled do
        DebugPrint("GhoulRangerAndGhoulStory loop iteration, enabled: " .. tostring(scriptStates.GhoulRangerAndGhoulStory.enabled))
        local allCleared = true
    ...
    DebugPrint("GhoulRangerAndGhoulStory script stopped, enabled: " .. tostring(scriptStates.GhoulRangerAndGhoulStory.enabled))
...
local function inspectFinishedScript()
    while scriptStates.InspectFinished.enabled do
        local inspectFinished = findObject("Inspect_Finished", "StringValue", player)
            or findObject("Inspect_Finished", "StringValue", playerGui)
            or findObject("Inspect_Finished", "StringValue", replicatedStorage)

        if inspectFinished then
            while inspectFinished and inspectFinished.Parent and scriptStates.InspectFinished.enabled do
                simulateClick()
                task.wait(1)
                inspectFinished = findObject("Inspect_Finished", "StringValue", player)
                    or findObject("Inspect_Finished", "StringValue", playerGui)
                    or findObject("Inspect_Finished", "StringValue", replicatedStorage)
            end
        end
...
local function autoUpgradeScript()
    ...
    local unitFrame = player.PlayerGui:WaitForChild("HUD")
        :WaitForChild("InGame")
...
local function voteNextScript()
    ...
    while scriptStates.VoteNext.enabled do
        local rewardsUI = playerGui:FindFirstChild("RewardsUI")
        if rewardsUI and rewardsUI.Enabled then
            -- ตรวจสอบว่าเป็น TokyoGhoul_Chapter10 หรือไม่
            if isPlayerInSomeMode("Story") then
                DebugPrint("VoteNext: Skipping in TokyoGhoul_Chapter10")
                task.wait(5) -- ข้ามการทำงานในด่านนี้
            else
...
-- ฟังก์ชัน Auto Leave (อัปเดตโดยใช้ VirtualInputManager เพื่อจำลองการคลิกปุ่ม Leave)
        ...
        if rewardsUI.Enabled and leaveButton.Visible then
            ...
            if scriptStates.AutoLeave.enabled and rewardsUI.Enabled then
                ...
                local success, err = pcall(function()
                    ...
                    DebugPrint("AutoLeave: Simulating click at position (" .. clickX .. ", " .. clickY .. ")")

                    -- จำลองการกดเมาส์ (MouseButton1 ลง)
                    VirtualInputManager:SendMouseButtonEvent(clickX, clickY, 0, true, leaveButton, 1)
                    task.wait(0.1) -- รอสั้นๆ เพื่อให้การกดลงทะเบียน
                    -- จำลองการปล่อยเมาส์ (MouseButton1 ขึ้น)
                    VirtualInputManager:SendMouseButtonEvent(clickX, clickY, 0, false, leaveButton, 1)
                end)
                if not success then
                    warn("AutoLeave: Failed to simulate Leave button click - " .. tostring(err))
                end
                -- รอจนกว่า RewardsUI จะปิด
    ...
    buttonData.button.MouseButton1Click:Connect(function()
        if scriptStates[scriptName].enabled then
            ...
            local scriptFunc = ({
                ...
                VoteNext = voteNextScript,
                RangerAndEasterEvent = rangerAndEasterEventScript,
                GogetaMadaraAndZ10 = gogetaMadaraAndZ10Script,
                GogetaAndZ10 = gogetaAndZ10Script,
                MadaraAndZ10 = MadaraAndZ10Script,
                AllRangerAndAllChallenge = allRangerAndAllChallengeScript,
                GhoulRangerAndGhoulStory = GhoulRangerAndGhoulStoryScript,
                AutoLeave = autoLeaveScript -- เพิ่ม AutoLeave
            })[scriptName]
